defmodule QaDashboard.Accounts.UserToken do
  ...
  schema "users_tokens" do
    ...
    belongs_to :user, QaDashboard.Accounts.User
  ...
  def build_session_token(user) do
    ...
    {token, %QaDashboard.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  defp build_hashed_token(user, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %QaDashboard.Accounts.UserToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       user_id: user.id
  ...
  def token_and_context_query(token, context) do
    from QaDashboard.Accounts.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in QaDashboard.Accounts.UserToken, where: t.user_id == ^user.id
  end
