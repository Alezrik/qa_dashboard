defmodule QaDashboard.AccountsTest do
  ...
  describe "user_ingest_tokens" do
    ...
    def user_ingest_token_fixture(attrs \\ %{}) do
      {:ok, user_ingest_token} =
        ...
        |> Accounts.create_user_ingest_token()

      user_ingest_token
    end

    test "list_user_ingest_tokens/0 returns all user_ingest_tokens" do
      user_ingest_token = user_ingest_token_fixture()
      assert Accounts.list_user_ingest_tokens() == [user_ingest_token]
    end

    test "get_user_ingest_token!/1 returns the user_ingest_token with given id" do
      user_ingest_token = user_ingest_token_fixture()
      assert Accounts.get_user_ingest_token!(user_ingest_token.id) == user_ingest_token
    end
    ...
    test "create_user_ingest_token/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_user_ingest_token(@invalid_attrs)
    end
    ...
    test "change_user_ingest_token/1 returns a user_ingest_token changeset" do
      user_ingest_token = user_ingest_token_fixture()
      assert %Ecto.Changeset{} = Accounts.change_user_ingest_token(user_ingest_token)
    end
  end
end
